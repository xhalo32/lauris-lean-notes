...
/-
[First-order logic with equality][first-order-logic-with-eq] imposes two axiom schemas, reflexivity and substitution principle, on equality.{margin}[Substitution principle is also called Leibniz's law.]
We explain how they correspond to the introduction rule and to a special case of the elimination rule of the inductive type `Eq`.

[first-order-logic-with-eq]: https://en.wikipedia.org/wiki/First-order_logic#Equality_and_its_axioms

Two basic facts about inductive types, constructor distinctness and injectivity, are proved using the substitution principle. We treat them in turn.


# Formation

Equality is encoded by the inductive type `Eq`.
-/
#print Eq

example (α : Sort u) (a b : α) : (a = b) = Eq a b := rfl
/-

We define our own version of `Eq`.
-/
...
We have made extensive use of `rfl`, which is simply the constructor `Eq.refl` of `Eq`, with both the arguments implicit.
-/
...
It proves reflexivity of equality.
-/
example (α : Sort u) (a : α) : a = a := rfl

example (α : Sort u) (a : α) : Eq' a a := rfl'
/-

The applicability of `rfl` as a proof is entirely determined by the [definitional equality][definitional-equality] implemented by the kernel, and `rfl'` behaves identically to `rfl` with respect to all its aspects, summarized below.
...
# Elimination
%%%
tag := "sec-substitution-principle"
%%%

Rather than using the recursor of `Eq` directly, equality is typically eliminated by using the substitution principle formulated as follows: if `a = b` and `P a` holds for a predicate `P`, then `P b` holds.{margin}[We could demonstrate the substitution principle just as well using our version `Eq'`, but `Eq` comes with the syntactic sugar `=` which improves readability.] We can prove it using the recursor
-/
example :
...
:= @Eq.rec
  ...
  h1 /- major premise -/
/-
The recursor permits any universe as the codomain of the motive, while the substitution principle is more special: it has the proposition `P b` as its codomain.
...
Setting the option `pp.explicit` to `true` makes `#print` show all implicit arguments.
-/
